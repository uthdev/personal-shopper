...
const envSchema = z.object({
  ...
  NODE_ENV: z
    ...
    .default('development'),
  MONGODB_URI: z.string().min(1, 'MONGODB_URI is required').optional().default('mongodb://localhost:27017/product_db'),
  DB_NAME: z.string().min(1, 'DB_NAME is required').optional().default('product_db'),
...
function validateEnv() {
  try {
    ...
    console.error('Product service environment validation failed:', error);
